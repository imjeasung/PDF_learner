...
document.addEventListener('DOMContentLoaded', function() {
    ...
    switch (currentPage) {
        ...
        case '':
            ...
            break;
        case 'upload.html':
            // upload.html에서 자체 초기화 함수(initUploadPageCustom) 사용
            break;
        case 'study.html':
...
async function deleteDocument(filename) {
    ...
    try {
        ...
        if (typeof loadDocuments === 'function') {
            await loadDocuments(); // 학습 페이지용
        }
        if (typeof loadUploadStats === 'function') {
            await loadUploadStats(); // 업로드 페이지용
        }
        if (typeof loadUploadedFiles === 'function') {
            await loadUploadedFiles(); // 업로드 페이지용
        }
...
async function downloadDocument(filename) {
    try {
        ...
        showMessage('파일 다운로드가 시작되었습니다.', 'success');
        
    } catch (error) {
        console.error('Download failed:', error);
        showMessage(`파일 다운로드에 실패했습니다: ${error.message}`, 'error');
    }
}
 ...
 */
function startLearning(filename) {
    window.location.href = `study.html?doc=${encodeURIComponent(filename)}`;
}
